...
export default class MyAbout extends Component {
  ...
  componentDidMount() {
    //当输入一个值后跳转到下一个Input框
    let { tele, currentTimes } = this.props.location.state
    currentTimes = currentTimes / 1000
    if (currentTimes) {
      let time = this.state.count - currentTimes

      if (time <= 0) {
        function sendMessageToContentScript(mes) {
          mes.type = 'cancelLoginTime'
        ...
        const sendMessageToContentBackgroundScript = (mes) => {
          ...
          chrome.runtime.sendMessage({ mes }, function (res) {
            ...
            if (res.code === 200) {
              ...
              const { autoFill, autoLogin, autoStore, lockedDelay, id } = plugin
              let { firstTimeLogin, nickName } = user
              const { loginUsername } = getPayloadObjFromJwt(token)
    ...
    return (
      <div className="home-reset-wrapper">
        <div className="error-message" style={{ display: this.state.show }}>
          <img src={iconFail} className="icon-fail" alt="pswNum-iconFail" />
