...
class PsdLibrary extends Component {
  ...
  componentDidMount() {
    ...
    const getData = async () => {
      ...
      const setList = (data) => {
        let sortArr = data.sort(arrSortMinToMax)
        if (data) {
          if (oldList && oldList.length > 0 && data && data.length > 0) {
            for (let i = 0; i < data.length; i++) {
              for (let j = 0; j < oldList.length; j++) {
                if (oldList[j].id == data[i].id) {
                  data.splice(i, 1)
                }
              }
            }
    ...
    return (
      <div className="psw-wrapper">
        ...
        <div className="home-search">
          <Input
            prefix={<img src={Search} className="icon-search" alt="Search" />}
